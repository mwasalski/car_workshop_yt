...
> **Pick the right shell first.** Cloud Shell runs either **Bash** or **PowerShell**
> (dropdown, top-left). The two use different syntax for variables — `VAR=value` in
> Bash, `$VAR = "value"` in PowerShell — so a Bash block pasted into PowerShell fails
> immediately with *"The term 'LOCATION=northeurope' is not recognized…"*. Both
> versions are below; the `az` commands themselves are identical.
...
az storage account create --name $SA --resource-group $RG --location $LOCATION --sku Standard_LRS  --kind StorageV2 --enable-hierarchical-namespace true --min-tls-version TLS1_2 --allow-blob-public-access false

# ---- 3. container ----
az storage container create --account-name $SA --name $CONTAINER
az storage container create --account-name $SA --name tables

# ---- 4. access connector (managed identity UC will use) ----
az extension add --name databricks
az databricks access-connector create --name $AC   --resource-group $RG  --location $LOCATION --identity-type SystemAssigned
...
# connector id    : /subscriptions/a4a98a3f-78f3-4a6a-a5ba-ae4fe50406e8/resourceGroups/rg-car-workshop/providers/Microsoft.Databricks/accessConnectors/ac-car-workshop
```

### PowerShell

Same steps, PowerShell syntax: `$VAR = "value"`, backtick `` ` `` for line
continuation (not `\`), and `Get-Random` instead of `$RANDOM`.

```powershell
# ---- variables (adjust once) ----
$LOCATION  = "northeurope" 
$RG = "rg-car-workshop"
$SA        = "sacarworkshop$(Get-Random -Maximum 99999)"  # globally unique, lowercase+digits
$AC        = "ac-car-workshop"
$CONTAINER = "landing"
...
Write-Host "connector id    : $(az databricks access-connector show -n $AC -g $RG --query id -o tsv)"
```
