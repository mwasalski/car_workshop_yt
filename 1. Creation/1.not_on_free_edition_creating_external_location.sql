...
--      --scope /subscriptions/<sub-id>/resourceGroups/rg-car-workshop/providers/Microsoft.Storage/storageAccounts/sacarworkshop
--
...
CREATE STORAGE CREDENTIAL IF NOT EXISTS car_workshop_adls_cred
  WITH AZURE_MANAGED_IDENTITY (
    ACCESS_CONNECTOR_ID = '/subscriptions/a4a98a3f-78f3-4a6a-a5ba-ae4fe50406e8/resourceGroups/rg-car-workshop/providers/Microsoft.Databricks/accessConnectors/ac-car-workshop'
  )
  COMMENT 'Managed identity credential for sacarworkshop storage account';
...
CREATE EXTERNAL LOCATION IF NOT EXISTS car_workshop_landing
  URL 'abfss://landing@sacarworkshop.dfs.core.windows.net/'
...
-- LIST 'abfss://landing@sacarworkshop.dfs.core.windows.net/';
...
-- any table location.
CREATE EXTERNAL VOLUME IF NOT EXISTS car_workshop.bronze.adls_landing
  LOCATION 'abfss://landing@sacarworkshop.dfs.core.windows.net/volumes/bronzelanding'
  COMMENT 'External volume on ADLS - raw file dump (parquet/csv/json)';

-- Access from notebooks exactly like a managed volume:
--   /Volumes/car_workshop/bronze/adls_landing/...
--   dbutils.fs.ls('/Volumes/car_workshop/bronze/adls_landing/')
--   df.write.parquet('/Volumes/car_workshop/bronze/adls_landing/some_dump/')

-- Landing zone for bronzeension snapshots: initial_bronzes.ipynb writes parquet
-- here (one folder per table); loading into car_workshop.bronze.* is a
-- separate ingestion step.
CREATE EXTERNAL VOLUME IF NOT EXISTS car_workshop.bronze.bronzelanding
  LOCATION 'abfss://landing@sacarworkshop.dfs.core.windows.net/volumes/landing'
  COMMENT 'External volume on ADLS - parquet snapshots of bronze source data';

-- -------------------------------------------------------------
-- 4. EXTERNAL TABLE (Delta table living on ADLS)
-- -------------------------------------------------------------
-- Example: external copy of a bronze table. DROP TABLE will NOT delete
-- the underlying files - clean the path manually if you recreate it.
CREATE TABLE IF NOT EXISTS car_workshop.bronze.sales_external (
  sale_id                  BIGINT,
  location_id              BIGINT,
  customer_id              BIGINT,
  employee_id              BIGINT,
...
USING DELTA
LOCATION 'abfss://landing@sacarworkshop.dfs.core.windows.net/tables/sales_external';
