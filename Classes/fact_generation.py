"""Daily bronze generator - logic extracted 1:1 from daily.ipynb.

`generate_day(target_date, scale_bronzeor, output_dir, bronzes)` writes one day of
parquet for all 13 bronze tables under `output_dir/<table>/` (hive-partitioned
...
reproduces identical rows (exception: payments.transaction_number uses
uuid4 - a pre-existing leak, tracked in the journal).

`bronzes` carries the FK pools; build it with `bronzes_from_frames` (notebook: from
spark.table reads; local lab: from parquet snapshots via `load_bronzes`).
"""
...
# rows per day at SCALE_bronzeOR = 1.0
DAILY_BASE_ROWS = {
    'work_orders':          2_000,
    'work_order_items':     6_000,
    'sales_transactions':  12_000,
    'sales_items':         36_000,
    'invoices':            10_000,
    'payments':            10_000,
    'inventory_movements': 20_000,
    'appointments':         2_500,
    'purchase_orders':        300,
    'purchase_order_items': 1_200,
    'customer_feedback':      800,
    'loyalty_program':      1_500,
    # employee_schedules: one row per employee per day (not scaled)
}
...
def bronzes_from_frames(locations, employees, customer_ids, vehicle_ids,
                     product_ids, service_ids, supplier_ids):
    """Derive every FK pool the generators need from raw bronze data.
    ...
    """
    for name, ids in [('customers', customer_ids), ('vehicles', vehicle_ids),
                      ('products', product_ids), ('services', service_ids),
                      ('suppliers', supplier_ids),
                      ('locations', locations['location_id'].to_numpy()),
                      ('employees', employees['employee_id'].to_numpy())]:
        if len(ids) == 0:
            raise RuntimeError(f'{name} is empty - generate bronze first')
...
def generate_day(target_date, scale_bronzeor, output_dir, bronzes):
    """Generate one day of all 13 bronze tables. Returns [{'table', 'rows'}, ...]."""
    TARGET_DATE = target_date
    ROWS = {table: max(int(n * scale_bronzeor), 1) for table, n in DAILY_BASE_ROWS.items()}
    ID_START = (TARGET_DATE - EPOCH).days * ID_BLOCK
    assert max(ROWS.values()) < ID_BLOCK, 'SCALE_bronzeOR too large for ID_BLOCK'
    ...
    customer_ids = bronzes['customer_ids']
    vehicle_ids = bronzes['vehicle_ids']
    product_ids = bronzes['product_ids']
    service_ids = bronzes['service_ids']
    supplier_ids = bronzes['supplier_ids']
    location_ids = bronzes['location_ids']
    workshop_locs = bronzes['workshop_locs']
    shop_locs = bronzes['shop_locs']
    employee_ids = bronzes['employee_ids']
    mechanic_ids = bronzes['mechanic_ids']
    seller_ids = bronzes['seller_ids']
    loc_mechanics = bronzes['loc_mechanics']
    loc_sellers = bronzes['loc_sellers']
    ...
    def write_bronze(table_name, total_rows, generate_chunk):
    ...
    write_bronze('work_orders', N_WO, gen_work_orders)
    write_bronze('work_order_items', ROWS['work_order_items'], gen_work_order_items)

    # ── shop - sales ────────────────────────────────────────────
    N_TRX = ROWS['sales_transactions']
    ...
    write_bronze('sales_transactions', N_TRX, gen_sales_transactions)
    write_bronze('sales_items', ROWS['sales_items'], gen_sales_items)
    ...
    write_bronze('invoices', N_INV, gen_invoices)
    write_bronze('payments', ROWS['payments'], gen_payments)

    # ── inventory & purchasing ──────────────────────────────────
    N_PO = ROWS['purchase_orders']
    po_ids = day_ids(N_PO)
    ...
    write_bronze('inventory_movements', ROWS['inventory_movements'], gen_inventory_movements)
    write_bronze('purchase_orders', N_PO, gen_purchase_orders)
    write_bronze('purchase_order_items', ROWS['purchase_order_items'], gen_purchase_order_items)
    ...
    write_bronze('appointments', ROWS['appointments'], gen_appointments)
    write_bronze('customer_feedback', ROWS['customer_feedback'], gen_customer_feedback)
    write_bronze('loyalty_program', ROWS['loyalty_program'], gen_loyalty_program)
    write_bronze('employee_schedules', len(employee_ids), gen_employee_schedules)  # 1 row per employee
